...
class SearchSourceConnectorBase(BaseModel):
    ...
    def validate_config_for_connector_type(
        ...
        elif connector_type == SearchSourceConnectorType.OBSIDIAN_CONNECTOR:
            # For OBSIDIAN_CONNECTOR, only allow vault_paths
            allowed_keys = ["vault_paths"]
            if set(config.keys()) != set(allowed_keys):
                raise ValueError(
                    f"For OBSIDIAN_CONNECTOR connector type, config must only contain these keys: {allowed_keys}"
                )

            # Ensure vault_paths is present and is a non-empty list
            vault_paths = config.get("vault_paths")
            if not isinstance(vault_paths, list) or not vault_paths:
                raise ValueError("vault_paths must be a non-empty list of strings")

            # Validate each vault path is a string
            for path in vault_paths:
                if not isinstance(path, str) or not path.strip():
                    raise ValueError("All vault paths must be non-empty strings")
