...
class ObsidianVaultPathsRequest(BaseModel):
    vault_paths: List[str] = Field(..., description="List of paths to Obsidian vaults")
...
# --- New Endpoint to list Obsidian Vault files ---
@router.post("/obsidian/vault-files/", response_model=Dict[str, Any])
async def list_obsidian_vault_files(
    vault_request: ObsidianVaultPathsRequest,
    user: User = Depends(current_active_user),  # Ensure the user is logged in
):
    """
    Fetches a list of markdown files from the provided Obsidian vault paths.
    The vault paths are used for this request only and are not stored.
    """
    try:
        # Initialize ObsidianVaultConnector with the provided paths
        obsidian_client = ObsidianVaultConnector(vault_paths=vault_request.vault_paths)

        # Get vault statistics
        vault_stats = obsidian_client.get_vault_stats()

        # Get list of files (limit to first 1000 for performance)
        all_files = obsidian_client.get_vault_files()

        return {
            "vault_stats": vault_stats,
            "total_files": len(all_files),
            "files": all_files[:1000],  # Limit response size
            "truncated": len(all_files) > 1000,
        }
    except ValueError as e:
        # Handle invalid vault path error specifically
        logger.error(f"Obsidian vault validation failed for user {user.id}: {str(e)}")
        raise HTTPException(status_code=400, detail=f"Invalid vault paths: {str(e)}")
    except Exception as e:
        logger.error(
            f"Failed to fetch Obsidian vault files for user {user.id}: {str(e)}"
        )
        raise HTTPException(
            status_code=500, detail="Failed to fetch Obsidian vault files."
        )
...
async def index_connector_content(
    ...
    try:
        ...
        if connector.connector_type == SearchSourceConnectorType.SLACK_CONNECTOR:
            ...
            background_tasks.add_task(
                run_obsidian_indexing_with_new_session,
                connector_id,
                search_space_id,
                str(user.id),
                indexing_from,
...
# --- Obsidian Vault Indexing Functions ---
async def run_obsidian_indexing_with_new_session(
    ...
    """Run Obsidian vault indexing with a new database session."""
    async with async_session_maker() as session:
...
async def run_obsidian_indexing(
    ...
    """Run Obsidian vault indexing."""
    try:
        logger.info(f"Starting Obsidian vault indexing for connector {connector_id}")

        # Call the indexing task with the corrected signature
        documents_processed, error_message = await index_obsidian_vault(
            session=session,
            connector_id=connector_id,
            search_space_id=search_space_id,
            user_id=user_id,
        ...
        if error_message:
            logger.error(
                f"Obsidian vault indexing failed for connector {connector_id}: {error_message}"
            )
        else:
            logger.info(
                f"Obsidian vault indexing successful for connector {connector_id}. Indexed {documents_processed} documents."
            )
            # Update the last indexed timestamp only on success
            await update_connector_last_indexed(session, connector_id)
            await session.commit()  # Commit timestamp update
        ...
        await session.rollback()
        logger.error(
            f"Critical error in run_obsidian_indexing for connector {connector_id}: {e}",
            exc_info=True,
        )
        # Optionally update status in DB to indicate failure
